...
def main(args, just_give_data: bool = False):
    ...
    if args.curation_method is not None:
        ...
        id_scores = scores_zarr[args.curation_method][args.curation_task][
            "id_scores"
        ]
        try:
            id_uids = id_zarr[args.curation_task]["id_indices"]
        ...
        print(f"Number of OOD samples: {np.sum(ood_scores > cutoff_value)}")
        print(f"Number of ID samples: {np.sum(id_scores > cutoff_value)}")
        print(
            f"Keeping {(all_scores >= cutoff_value).mean():.2%} of the data ({(id_scores >= cutoff_value).mean():.2%} id / {(ood_scores >= cutoff_value).mean():.2%} ood)"
        )
        selected_ood_idx = set(ood_uids[ood_scores >= cutoff_value])
        selected_id_idx = set(id_uids[np.where(id_scores >= cutoff_value)[0]])
        ...
        assert (
            args.indistribution_data_sampling_rate is None
        ), "Indistribution data sampling rate already specified"
        args.indistribution_data_sampling_rate = p_id

        def id_filter(sample):
            return sample["__key__"] in selected_id_str

        if ood_filter is None:
            print("Setting ood filter.")

            def ood_filter(sample):
                uid = json.loads(sample["json"].decode("utf-8"))["uid"]
                return uid in selected_ood_idx
        else:
            print("Setting combined ood filter with excluded uids.")

            def ood_filter(sample):
                uid = json.loads(sample["json"].decode("utf-8"))["uid"]
                return (uid in selected_ood_idx) & ood_filter(sample)
    else:
        id_zarr = zarr.open("/raid/pdpl/id_downstream_idx.zarr", mode="r")
